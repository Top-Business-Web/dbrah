...
class OrderController extends Controller
{
    ...
    public function storeOrder(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'details.*.products.*.id' => 'required|exists:products,id',
            'details.*.products.*.category_id' => ["required",Rule::exists('categories','id')
                ->where('level',1)],
            'details.*.products.*.sub_category_id' => ["required",Rule::exists('categories','id')
                ->where('level',2)],
            'details.*.products.*.qty' => 'required',
        ]);
        if ($validator->fails()) {
            $code = $this->returnCodeAccordingToInput($validator);
        ...
        foreach($request->details as $key=>$data)
        {
            ...
            foreach ($data['products'] as $key => $value3) {
